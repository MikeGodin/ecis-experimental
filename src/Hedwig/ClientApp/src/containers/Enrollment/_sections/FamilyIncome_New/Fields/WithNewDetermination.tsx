...
export const WithNewDetermination = ({
	...
	useEffect(() => {
		if (shouldCleanUp && newDet.value != undefined) {
			...
			console.log(
				'is create',
				idx(data, _ => _.child.family.determinations)
			);
			updateData(_data => {
				return produce<Enrollment>(_data, draft =>
					set(draft, newDet.path, { id: 0, numberOfPeople: 0 })
				);
			});
			// setRerender(r => r + 1);
		}
	...
	return <React.Fragment>{determinationFields}</React.Fragment>;
	// options:
